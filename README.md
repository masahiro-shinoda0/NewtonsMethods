...
次に，すべての自然数 $`n`$ に対して，$`A_n>\sqrt{2}`$ であることを数学的帰納法を用いて証明する。
##### (I)
$`n=1`$ のとき。このとき，$`A_1 = 2 > \sqrt{2}`$ であるからみたす。
...
(I), (II)より，すべての自然数 $`n`$ に対して，$`A_n>\sqrt{2}`$ であることが示された。

上の結果より，$`A_n>\sqrt{2}=\alpha`$ ，よって$`\frac{1}{A_n}-\frac{1}{\alpha}<0`$であるから，以下の不等式を得る。
